...
// Bentuk record mengikuti OmsetKumulatif, namun sumber data dari 'omset_pendapatan_accrual'
...
const OmsetAccrualKumulatif: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        const raw = await kvStore.get('omset_pendapatan_accrual');
    ...
    (async () => {
      await refresh();
      try {
        const sub = kvStore.subscribe('omset_pendapatan_accrual', () => { try { refresh(); } catch {} });
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', maxHeight: 'calc(100vh - 64px)', overflowY: 'auto', p: 3, boxSizing: 'border-box', flexDirection: 'column' }}>
      <Box sx={{ width: '80%', borderRadius: 2, boxShadow: 2, flexDirection: 'column', p: 3, mb: 3 }}>
        <Typography variant='h6' sx={{ mb: 2, fontWeight: 'bold' }}>Omset Accrual Kumulatif</Typography>
        ...
        <TableExportToolbar title="Omset Accrual Kumulatif" fileBaseName="omset-accrual-kumulatif" tableRef={tableRef} />
        <TableContainer component={Paper} sx={{ width: '100%', overflowX: 'auto', mt: 3 }}>
          <Table sx={{ minWidth: 650 }} aria-label='omset-accrual-kumulatif' ref={tableRef}>
            <TableHead>
              <TableRow>
        ...
        <TableExportToolbar title="Omset by Trend" fileBaseName="omset-accrual-trend" tableRef={trendRef} />
        <TableContainer component={Paper} sx={{ width: '100%', overflowX: 'auto', mt: 2 }}>
          <Table size="small" aria-label='omset-accrual-trend' ref={trendRef}>
            <TableHead>
              <TableRow>
        ...
        <Box sx={{ mt: 4 }}>
          ...
          {dailySeries.labels.length > 0 && (
            <>
              <Typography variant='subtitle1' sx={{ mt: 4, mb: 1, fontWeight: 600 }}>Omset Accrual Kumulatif per Hari + Trendline</Typography>
              <ResponsiveContainer width="100%" height={280}>
                <ComposedChart data={dailySeries.labels.map((d, i) => ({ date: d, value: dailySeries.cumOmset[i] }))}>
                  ...
                  <RLegend />
                  <RBar dataKey="value" name="Omset Accrual Kumulatif" fill="#6d4c41" />
                  {(() => {
...
export default OmsetAccrualKumulatif;
