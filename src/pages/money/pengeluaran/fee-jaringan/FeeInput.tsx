...
const FeeInput: React.FC = () => {
  ...
  useEffect(() => {
    let mounted = true;
    const refresh = async () => {
      try { const raw = await kvStore.get(STORAGE_KEY); const list = Array.isArray(raw) ? raw as Fee[] : (raw ? JSON.parse(String(raw)) : []); if (mounted) setItems(list); } catch { if (mounted) setItems([]); }
    };
    (async () => {
      await refresh();
      try {
  ...
  useEffect(() => {
    (async () => { try { await kvStore.set(STORAGE_KEY, items); } catch {} })();
  }, [items]);
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', maxHeight: 'calc(100vh - 64px)', overflowY: 'auto', p: 3, boxSizing: 'border-box', flexDirection: 'column' }}>
      <Box sx={{ width: '80%', borderRadius: 2, boxShadow: 2, flexDirection: 'column', p: 3, mb: 3 }}>
        <Typography variant='h6' sx={{ mb: 2, fontWeight: 'bold' }}>Fee Jaringan - Input</Typography>
