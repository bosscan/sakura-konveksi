...
const OngkirReport: React.FC = () => {
  const [month, setMonth] = useState<string>(new Date().toISOString().slice(0,7));
  const [data, setData] = useState<Row[]>([]);
  useEffect(() => {
    let mounted = true;
    const refresh = async () => { try { const raw = await kvStore.get(STORAGE_KEY); const list = Array.isArray(raw) ? (raw as Row[]) : (raw ? JSON.parse(String(raw)) : []); if (mounted) setData(list); } catch { if (mounted) setData([]); } };
  ...
  const perDate = useMemo(() => {
    ...
    filtered.forEach(i => map.set(i.date, (map.get(i.date) || 0) + (i as any).amount));
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', maxHeight: 'calc(100vh - 64px)', overflowY: 'auto', p: 3, boxSizing: 'border-box', flexDirection: 'column' }}>
      <Box sx={{ width: '80%', borderRadius: 2, boxShadow: 2, flexDirection: 'column', p: 3 }}>
        <Typography variant='h6' sx={{ mb: 2, fontWeight: 'bold' }}>Ongkir - Report</Typography>
        ...
        <TableExportToolbar title="Ongkir - Report" fileBaseName="ongkir-report" tableRef={tableRef} />
