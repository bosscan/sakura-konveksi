...
type StockItem = {
  id: string; date: string; code: string; name: string; category: string; unit: string; qtyIn: number; qtyOut: number; price: number; supplier: string; note: string;
};
const STORAGE_KEY = 'material_logistik2';
import kvStore from '../../../lib/kvStore';

export default function Logistik2() {
  const [items, setItems] = useState<StockItem[]>([]);
  useEffect(() => {
    let mounted = true;
    (async () => {
      try { const raw = await kvStore.get(STORAGE_KEY); const list = Array.isArray(raw) ? raw : (raw ? JSON.parse(String(raw)) : []); if (mounted) setItems(list as StockItem[]); } catch { if (mounted) setItems([]); }
    })();
  ...
  return (
    <Box sx={{ p:3 }}>
      <Typography variant="h6" fontWeight={700} sx={{ mb:2, textAlign:'center' }}>Input Stock - Logistik 2</Typography>
      <Paper sx={{ p:2, mb:2 }}>
