...
export default function LembarJahit() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                ...
                setSnack({ open: true, message: 'Gagal memuat data lembar jahit.', severity: 'error' });
            }
        };
        load();
    }, [spkId]);
    ...
    const handleConfirm = async () => {
        try {
            ...
            setOpen(false);
            setSnack({ open: true, message: updated ? 'Status divisi Jahit diset selesai. Diteruskan ke antrian.' : 'ID SPK tidak ditemukan.', severity: updated ? 'success' : 'info' });
            if (updated) setTimeout(() => navigate('/method/update-divisi/jahit/antrian'), 600);
        } catch {
            setOpen(false);
