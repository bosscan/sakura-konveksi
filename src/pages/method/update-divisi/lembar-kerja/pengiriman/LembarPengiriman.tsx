...
export default function LembarPengiriman() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                ...
                setSnack({ open: true, message: 'Gagal memuat data lembar pengiriman.', severity: 'error' });
            }
        };
        load();
    ...
    const handleConfirm = async () => {
        try {
            ...
            setOpen(false);
            setSnack({ open: true, message: updated ? 'Status divisi Pengiriman diset selesai. Diteruskan ke antrian.' : 'ID SPK tidak ditemukan.', severity: updated ? 'success' : 'info' });
            if (updated) setTimeout(() => navigate('/method/update-divisi/pengiriman/antrian'), 600);
        } catch {
            setOpen(false);
