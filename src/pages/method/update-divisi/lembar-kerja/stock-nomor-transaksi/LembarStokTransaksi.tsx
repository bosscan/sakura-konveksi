...
import { isSpkInDivisionQueue, markSelesai } from "../../../../../lib/pipelineHelpers";
import { kvStore } from "../../../../../lib/kvStore";

export default function LembarStokTransaksi() {
    ...
    useEffect(() => {
        if (!spkId) return;
        const ok = isSpkInDivisionQueue(spkId, 'stock-no-transaksi');
        if (!ok) {
            setSnack({ open: true, message: 'SPK ini belum masuk ke antrian divisi tersebut.', severity: 'info' });
            setTimeout(() => navigate('/method/update-divisi/stok-transaksi/antrian'), 700);
        }
    }, [spkId, navigate]);
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                ...
                setSnack({ open: true, message: 'Gagal memuat data lembar stok transaksi.', severity: 'error' });
            }
        };
        load();
        ...
        <Box sx={{
            ...
            <Box sx={{
                ...
                <Typography variant="h6" fontWeight={700} align="center" sx={{ width: '100%' }}>
                    Lembar Kerja Divisi Stok Transaksi {spkId ? `(SPK: ${spkId})` : ''}
                </Typography>
                ...
                >
                    ...
                    <Box sx={{ width: { xs: '100%', md: 480 }, ml: { xs: 0, md: 2} }}>
                        <TableContainer component={Paper} sx={{ width: '100%' }}>
                            <Table
                                aria-label="lembar-stok-transaksi"
                                size="small"
                                sx={{
