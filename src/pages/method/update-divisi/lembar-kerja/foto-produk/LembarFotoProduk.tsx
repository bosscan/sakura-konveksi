...
export default function LembarFotoProduk() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                ...
                setSnack({ open: true, message: 'Gagal memuat data lembar foto produk.', severity: 'error' });
            }
        };
        load();
    ...
    const handleConfirm = async () => {
        try {
            ...
            setOpen(false);
            setSnack({ open: true, message: updated ? 'Status divisi Foto Produk diset selesai. Diteruskan ke antrian.' : 'ID SPK tidak ditemukan.', severity: updated ? 'success' : 'info' });
            if (updated) setTimeout(() => navigate('/method/update-divisi/foto-produk/antrian'), 600);
        } catch {
            setOpen(false);
