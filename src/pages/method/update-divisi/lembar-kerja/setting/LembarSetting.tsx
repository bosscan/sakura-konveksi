...
export default function LembarSetting() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                ...
                setSnack({ open: true, message: 'Gagal memuat data lembar setting.', severity: 'error' });
            }
        };
        load();
    }, [spkId]);
    ...
    const handleConfirm = async () => {
        try {
            ...
            setOpen(false);
            setSnack({ open: true, message: updated ? 'Status divisi Setting diset selesai. Diteruskan ke antrian.' : 'ID SPK tidak ditemukan.', severity: updated ? 'success' : 'info' });
            if (updated) setTimeout(() => navigate('/method/update-divisi/setting/antrian'), 600);
        } catch {
            setOpen(false);
        ...
        <Box sx={{
            ...
            <Box sx={{
                ...
                <Typography variant="h6" fontWeight={700} align="center" sx={{ width: '100%' }}>
                    Lembar Kerja Divisi Setting {spkId ? `(SPK: ${spkId})` : ''}
                </Typography>
