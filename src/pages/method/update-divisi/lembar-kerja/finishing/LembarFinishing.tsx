...
export default function LembarFinishing() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            if (!eligible) {
                alert('SPK belum masuk antrian Finishing. Pastikan alur sebelumnya selesai.');
            ...
            try {
                ...
                setSnack({ open: true, message: 'Gagal memuat data lembar finishing.', severity: 'error' });
            }
        };
        load();
    ...
    const handleConfirm = async () => {
        try {
            ...
            setOpen(false);
            setSnack({ open: true, message: updated ? 'Status divisi Finishing diset selesai. Diteruskan ke antrian.' : 'ID SPK tidak ditemukan.', severity: updated ? 'success' : 'info' });
            if (updated) setTimeout(() => navigate('/method/update-divisi/finishing/antrian'), 600);
        } catch {
            setOpen(false);
