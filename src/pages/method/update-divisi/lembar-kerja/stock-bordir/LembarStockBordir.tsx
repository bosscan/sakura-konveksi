...
export default function LembarStokBordir() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            // Gate: must be in Stok Bordir queue
            // Avoid auto-redirect on initial false. Recheck via kv_store and only show info.
            let eligible = isSpkInDivisionQueue(spkId, 'stock-bordir');
            if (!eligible) {
                try {
                    ...
                    // Stock Bordir: after desain-produksi & cutting-pola finished, and stock-bordir not yet done
                    eligible = !!it && done('selesaiDesainProduksi') && done('selesaiCuttingPola') && !done('selesaiStockBordir');
                } catch {}
            }
            if (!eligible) {
                alert('SPK belum masuk antrian Stok Bordir. Pastikan alur sebelumnya selesai.');
                navigate('/method/update-divisi/stock-bordir/antrian');
                return;
            }
            ...
            try {
                ...
                setSnack({ open: true, message: 'Gagal memuat data lembar stok bordir.', severity: 'error' });
            }
        };
        load();
    ...
    const handleConfirm = async () => {
        try {
            ...
            setOpen(false);
            setSnack({ open: true, message: updated ? 'Status divisi Stok Bordir diset selesai. Diteruskan ke antrian.' : 'ID SPK tidak ditemukan.', severity: updated ? 'success' : 'info' });
