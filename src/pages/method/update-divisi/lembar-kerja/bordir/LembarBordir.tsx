...
export default function LembarBordir() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            let eligible = isSpkInDivisionQueue(spkId, 'bordir');
            if (!eligible) {
                try {
                    ...
                    // Bordir eligible if stok-bordir done and bordir not yet done
                    eligible = !!it && done('selesaiStockBordir') && !done('selesaiBordir');
                } catch {}
            }
            if (!eligible) {
    ...
    const handleConfirm = async () => {
        try {
            ...
            setOpen(false);
            setSnack({ open: true, message: updated ? 'Status divisi Bordir diset selesai. Diteruskan ke antrian.' : 'ID SPK tidak ditemukan.', severity: updated ? 'success' : 'info' });
            if (updated) setTimeout(() => navigate('/method/update-divisi/bordir/antrian'), 600);
        } catch {
            setOpen(false);
