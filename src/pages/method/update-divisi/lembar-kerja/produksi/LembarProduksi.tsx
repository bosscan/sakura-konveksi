import { Box, Typography, Paper, TableContainer, TableHead, TableRow, TableCell, Table, Card, CardContent, TableBody, Checkbox, Button, Modal, Snackbar, Alert, Stack, Chip } from "@mui/material";
...
export default function LembarProduksi() {
    ...
    const [cdr, setCdr] = useState<{ file?: string | null; filename?: string } | null>(null);
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                ...
                const spkOrdersMap: Record<string, AnyRec> = designSnapRaw && typeof spkOrdersRaw === 'object' ? spkOrdersRaw as any : (spkOrdersRaw ? JSON.parse(String(spkOrdersRaw)) : {});
                ...
                let resolvedMock: string | undefined = design?.worksheet?.mockup?.file || designNext?.worksheet?.mockup?.file || undefined;
                if (!resolvedMock) {
                ...
                const resolvedCdr = (designNext?.worksheet?.cdr) || (design?.worksheet?.cdr) || ((snap as AnyRec)?.worksheet?.cdr) || null;
                if (mounted) setCdr(resolvedCdr || null);
                ...
                // kvStore-only: on error, clear state and notify
                if (mounted) {
                    setItems([]);
                    setRowChecks([]);
                    setMockupUrl('');
                ...
                setSnack({ open: true, message: 'Gagal memuat data lembar produksi.', severity: 'error' });
            }
        };
        load();
    }, [spkId]);
        ...
        <Box sx={{
            ...
            <Box sx={{
                ...
                <Typography variant="h6" fontWeight={700} align="center" sx={{ width: '100%' }}>
                    Lembar Kerja Divisi Desainer Produksi {spkId ? `(SPK: ${spkId})` : ''}
                </Typography>
                ...
                >
                    ...
                    <Box sx={{ width: { xs: '100%', md: 480 }, ml: { xs: 0, md: 2} }}>
                        ...
                        {/* File CDR uploaded by Pra Produksi designer */}
                        <Box sx={{ mt: 1.5 }}>
                            <Typography variant="body2" sx={{ fontWeight: 600, mb: 0.5 }}>File CDR</Typography>
                            {cdr?.file ? (
                                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} alignItems={{ xs: 'stretch', sm: 'center' }}>
                                    <Chip size="small" label={cdr?.filename || 'file.cdr'} />
                                    <Button variant="outlined" onClick={() => {
                                        try {
                                            const a = document.createElement('a');
                                            a.href = cdr.file!;
                                            a.download = `${spkId || 'desain'}-${cdr.filename || 'file'}.cdr`;
                                            a.click();
                                        } catch {}
                                    }}>
                                        Download CDR
                                ...
                                <Typography variant="body2" color="text.secondary">Tidak ada file CDR</Typography>
                            )}
                        </Box>
                    </Box>
                </Box>
